...
Pass DenseToSparse(const Array<ObjectRef>& weight_name,
  ...
  runtime::TypedPackedFunc<Function(Function, IRModule, PassContext)> pass_func =
      [=](Function f, IRModule m, PassContext pc) {
        ...
        auto f1 = Function(sparse_params, f0->body, f0->ret_type, f0->type_params, f0->attrs, Span(), f0->index);
        Array<Var> params = FreeVars(f1);
        for (const auto& var : sparse_params) {
          params.push_back(var);
        }
        return Function(params, f1->body, f1->ret_type, f1->type_params, f1->attrs, Span(), f1->index);
      };
